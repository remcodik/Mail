...
from flask import Flask, jsonify, redirect, request, send_file, session, url_for, send_from_directory
...
@app.before_request
def require_gmail_auth():
    """Block all /api/* requests when Gmail is not authenticated."""
...
def auth_status():
    """Public endpoint — lets the UI check auth state without a 401."""
    authenticated = auth.get_credentials() is not None
...
def api_wallet(message_id: str):
    """Generate and download a .pkpass file for a ticket email."""
    email = gmail.get_email(message_id)
    ticket_info = ai.detect_ticket(email)
    if not ticket_info:
        return jsonify({"error": "No ticket information found in this email"}), 400

    pkpass_buf = wallet.generate_pkpass(ticket_info, email)
    filename = f"mailai-{message_id[:8]}.pkpass"
    return send_file(
        pkpass_buf,
