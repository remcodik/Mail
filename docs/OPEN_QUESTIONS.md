...
| Q15 | **Propose meeting for agenda** | ✅ Built (demo): email detail offers "+ Propose meeting for agenda" (flagged when scheduling is detected) → slot chooser → an agenda entry on the Tasks screen with an **Add to Google Calendar** link + a proposal reply (*Send proposal* / *Remove*). Live wants a real Calendar/`generate_appointment_proposal` + free-slot lookup. | Done (demo); live Calendar follow-up |
| Q17 | **Agenda ↔ Google Calendar** | ✅ Built (demo): the Tasks agenda has an **Open Google Calendar** link and a **calendar selector** — toggle chips to add more calendars (Personal/Work/Family/Holidays/custom) into the view; events from selected calendars display inline and the choice persists. Live reads the user's real calendars + events via the **Google Calendar API** (extra OAuth scope) and, ideally, two-way (create events from proposals). | Done (demo); live Calendar API follow-up |
...
- **Propose meeting for agenda** — email detail can turn a scheduling mail into an agenda appointment with an Add-to-Google-Calendar link (parallel to "create task").
- **Agenda ↔ Google Calendar** — Open-Google-Calendar link + a calendar selector to view additional calendars inline (persists).
