...
function renderEmailList() {
  ...
  filtered.forEach(email => {
    ...
    // Long press → reveal swipe actions
    let pressTimer;
...
function openEmailDetail(email) {
  ...
  if (email.draft_reply) {
    html += `
      <div class="reply-card">
        ...
        <div class="reply-text" id="reply-text">${escHtml(email.draft_reply)}</div>
        <div class="reply-actions">
          <button class="btn btn-primary" id="send-reply-btn">Send</button>
          <button class="btn btn-secondary" id="edit-reply-btn">Edit</button>
  ...
  if (email.appointment_proposal) {
    html += `
      <div class="reply-card">
        ...
        <div class="reply-text">${escHtml(email.appointment_proposal)}</div>
        <div class="reply-actions">
  ...
  if (email.ticket_info) {
    ...
    html += `
      <div class="detail-summary-card">
        <h3>&#127903; ${t.type || 'Ticket'}</h3>
        <p>${escHtml(t.event || '')} ${t.date ? `· ${t.date}` : ''} ${t.seat ? `· Seat: ${t.seat}` : ''}</p>
        <button class="btn btn-primary" style="margin-top:10px;">Add to Wallet</button>
      </div>
    `;
  }
  ...
  document.getElementById('send-reply-btn')?.addEventListener('click', () => {
    const body = document.getElementById('reply-text')?.textContent || '';
  ...
  document.getElementById('send-appt-btn')?.addEventListener('click', () => {
    const body = document.querySelector('.reply-card .reply-text')?.textContent || '';
    api(`/api/email/${email.id}/send-reply`, {
      method: 'POST',
      body: JSON.stringify({ body }),
...
// ── Newsletter tab override ────────────────────────────────────────────────
// When the newsletter tab is active, load from /api/newsletters directly
const origRenderEmailList = renderEmailList;

function renderEmailListOverride() {
  if (state.tab === 'newsletter') {
    renderNewsletterScreen();
  } else if (state.tab === 'awaiting_reply') {
    renderWaitingScreen();
  } else {
    origRenderEmailList();
  }
...
// Override renderEmailList reference
window.renderEmailList = renderEmailListOverride;

// Also patch the tab listeners to call the override
document.querySelectorAll('.tab').forEach(btn => {
...
(function init() {
  // Pre-load version
  api('/api/version').then(v => { state.version = v; });
  renderScreen();
})();
