...
(function () {
  ...
  function viewArchive(spec){
    ...
    var snoozeSec = sn.length ? '<div class="seghead" style="padding-left:14px">Snoozed · '+sn.length+'</div><div class="list" style="padding-top:0">'
      + sn.map(function(m){ return '<div class="arow">'+cardHTML(m)+'<button class="btn arow-del" data-act="unsnooze" data-id="'+m.id+'">Wake now'+(m.snoozeUntil?(' · was '+esc(m.snoozeUntil)):'')+'</button></div>'; }).join('') + '</div>' : '';
